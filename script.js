...
function initCalculator() {
    ...
    const procedureList = {
        ...
        'FORL/기타': { cat: '발치/제거', items: [
            ...
            { l: '잇몸종양(<1cm)', v: 110000 }, { l: '잇몸종양(>1cm)', v: 220000 }, { l: '조직검사', v: 187000 }, { l: '함치성낭제거', v: 220000 }
        ]},
    ...
    window.addEventListener('beforeunload', (e) => {
        if (isChartDirty) { 
            e.preventDefault(); 
            e.returnValue = '변경되지 않은 사항이 저장되지 않을 수 있습니다.'; 
        }
    });
...
function copyCalculatorDataTo(targetId) {
    ...
    // Step 2: 복제된 데이터에서 "추가 처치" 부분의 불필요한 행들을 정리합니다.
    ...
    // [수정됨] 치아 차트의 빈 행을 숨기는 로직을 제거하여 항상 전체 테이블이 보이도록 합니다.
    // 이전에 이 위치에 있던 `clonedArea.querySelectorAll('.main-container tr').forEach` 블록을 삭제했습니다.
    ...
    // Step 5: 복제된 노드의 자식 요소들을 대상 영역으로 이동시킵니다.
