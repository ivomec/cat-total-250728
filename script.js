/*
  [최종 수정] HTML 구조를 동적으로 생성하도록 변경하여 ID를 찾지 못하는 오류를 해결했습니다.
  '병원소개' 탭의 레이아웃 변경 및 '자랑' 섹션 내용 업데이트를 모두 반영했습니다.
...
function populateContent(data) {
    ...
    if (data.main) {
        ...
        mainContainer.innerHTML = `
            ...
            <section class="pride-section" id="main-pride" style="background:#e3f2fd; padding: 25px; border-radius: 20px; margin-top: 30px;">
                <h2 style="color:#0277bd">${data.main.pride.title}</h2>
                ${prideItemsHTML}
            </section>
            <section class="important-notice" style="margin-top: 30px;">
                <h2 id="main-notice-title">${data.main.notice.title}</h2>
...
function initCalculator() {
    ...
    function updateTreatmentSummary() {
        ...
        page.querySelectorAll('.procedure-select').forEach(select => {
            ...
            if (!selectedOption || (select.value === '0' && selectedOption.text !== '모니터링') || select.value === 'disabled') return;
