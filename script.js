/*
  [v2.0 최종 업데이트 내역]
  - 계산기 로직: 강아지 차트의 안정적인 최신 로직으로 전면 교체
  - 데이터 적용: 고양이 치아 공식(30개) 및 수술비용 완벽 적용
  - 기능 동기화: 강아지 차트의 상세 자동계산 추가처치 내역 전체 이식
  - 기능 복구: 예상비용/보호자용 탭 기능 정상화 및 치아 이미지 자동 삽입
  - UI/UX 개선: 모니터링 항목 강조, 유튜브 링크 버튼화, 추가처치 목록 높이 조절
*/
...
function initCalculator() {
    ...
    const procedureList = {
        ...
        '기본 발치': { cat: '발치/제거', items: [ { l: '뿌리 1개', v: 22000, r: [1] }, { l: '뿌리 2개', v: 66000, r: [2] }, { l: '대구치/열육치', v: 88000, r: [3], t: ['108', '208', '409', '309'] } ]},
        '수술 발치': { cat: '발치/제거', items: [ { l: '뿌리 1개', v: 44000, r: [1] }, { l: '뿌리 2개', v: 120000, r: [2] }, { l: '대구치', v: 165000, t: ['409', '309'] }, { l: '열육치(PM4)', v: 220000, t: ['108', '208'] }, { l: '상악 송곳니', v: 220000, t: ['104', '204'] }, { l: '하악 송곳니', v: 270000, t: ['404', '304'] } ]},
        '유치 발치': { cat: '발치/제거', items: [ { l: '일반', v: 22000, tag: 'deciduous' }, { l: '송곳니(X-ray 포함)', v: 33000, t: ['104', '204', '404', '304'], tag: 'deciduous' }, { l: '송곳니(수술)', v: 66000, t: ['104', '204', '404', '304'], tag: 'deciduous' } ]},
        'FORL': { cat: '발치/제거', items: [ { l: '치관절제술', v: 44000 }, { l: '흡수치근제거(1개)', v: 88000, r: [1] }, { l: '흡수치근제거(2개)', v: 140000, r: [2, 3] } ]},
        '기타수술': { cat: '발치/제거', items: [ { l: '잔존치근(1개)', v: 77000, r: [1] }, { l: '잔존치근(2개)', v: 120000, r: [2, 3] }, { l: '잔존치근(상악송곳니)', v: 250000, t: ['104', '204'] }, { l: '잔존치근(하악송곳니)', v: 300000, t: ['404', '304'] }, { l: '잇몸종양(<1cm)', v: 110000 }, { l: '잇몸종양(>1cm)', v: 220000 }, { l: '조직검사', v: 170000 }, { l: '항생제감수성검사', v: 143000 }, { l: '함치성낭제거', v: 220000 } ]},
        '보존/치주': { cat: '신경/보존 치료', items: [ { l: 'VPT(송곳니)', v: 450000, t: ['104', '204', '404', '304'] }, { l: '신경치료(송곳니)', v: 770000, t: ['104', '204', '404', '304'] }, { l: '레진(Small)', v: 55000 }, { l: '레진(송곳니)', v: 110000, t: ['104', '204', '404', '304'] }, { l: '미노클린', v: 22000 }, { l: '치근활택술', v: 45000 } ]}
    };
    ...
    function populateAdditionalTreatments() {
        ...
        const treatmentsByCategory = [
             ...
             { category: '🩹 통증 관리', items: [ { id: 'pain_opioid_iv', name: '마약성 진통 혈관주사' }, { id: 'pain_24hr_injection', name: '24시간 지속 진통 주사' }, { id: 'pain_cri', name: '무통 주사' }, { id: 'pain_patch', name: '마약성 진통패치' } ], side: 'left' },
             { category: '🚀 회복 촉진', items: [ { id: 'recovery_injection', name: '항생/소염 주사' }, { id: 'laser_therapy', name: '레이저 치료' }, { id: 'fluoride', name: '불소 도포' } ], side: 'right' },
             { category: '🏡 홈케어', items: [ { id: 'medication', name: '내복약 (1일2회)' }, { id: 'liquid_analgesic_nsaid', name: '액상 진통제(NSID)'}, { id: 'hexidine_spray', name: '헥시딘 스프레이'}, { id: 'steroid_ointment', name: '구강항생 스테로이드연고'}, { id: 'coating_spray', name: '구강점막코팅스프레이'}, { id: 'paradont_gel', name: '파라돈 겔'}, { id: 'probiotics', name: '구강 유산균'}, { id: 'neck_collar', name: '넥카라' } ], side: 'right' }
        ];
    ...
    function updateAdditionalOptions() {
         ...
         page.querySelectorAll('.additional-treatments-container select').forEach(control => {
            ...
            // 강아지 차트의 로직을 그대로 적용
            if (itemId === 'health-check' && weight > 0) { add('고양이 안심+ 플랜', 199000); add('7세미만 으른냥 플랜', 299000); add('7세이상 어르신냥 VIP', 449000); }
            if (itemId === 'scaling-package' && weight > 0) { let price = (weight < 5) ? 239000 : (weight < 10) ? 299000 : 389000; add(`스케일링 패키지`, price); add(`스케일링(타병원검사)`, price + 100000); }
            ...
            if (itemId === 'medication' && weight > 0) { let p=3300; for(let d=1; d<=7; d++) add(`${d}일분`, p * d); }
    ...
    function updateTotalCost() {
        ...
        page.querySelectorAll('.additional-treatments-container .cost').forEach(cell => {
            if (cell.closest('tr').classList.contains('selected-row')) {
    ...
    page.addEventListener('input', (e) => { isChartDirty = true; if (e.target.matches('.notes')) updateRowHighlight(e.target.closest('tr')); if (e.target.matches('#patient-weight-calc')) { updateTotalCost(); populateAdditionalTreatments(); } if (e.target.matches('#patient-name-calc, #visit-date-calc')) page.querySelector('.dynamic-chart-title').textContent = `${page.querySelector('#visit-date-calc').value} ${page.querySelector('#patient-name-calc').value || '환자'}의 치과 차트`; });
...
function copyCalculatorDataTo(targetId) {
    ...
    clonedArea.querySelectorAll('.additional-treatments-container tr[style*="display: none;"]').forEach(row => { let next = row.nextElementSibling; let allHidden = true; while (next && !next.querySelector('td[colspan="3"]')) { if (next.style.display !== 'none') { allHidden = false; break; } next = next.nextElementSibling; } if (allHidden) row.style.display = 'none'; });
    ...
    toothFormulaImage.src = "https://raw.githubusercontent.com/ivomec/image/main/%EC%B9%98%EC%B9%9D1.jpg?raw=true";
    toothFormulaImage.alt = "강아지/고양이 치아 모식도";
...
function generateGuardianComments(clonedArea) {
    ...
    clonedArea.querySelectorAll('.procedure-select').forEach(select => { if (select.value !== '0') { const cat = select.options[select.selectedIndex].dataset.category; if (cat === '발치/제거') careAdviceCategories.add('EXTRACTION'); if (cat === '신경/보존 치료') careAdviceCategories.add('RESIN'); } });
    if (clonedArea.querySelector('[data-item-id="medication"]')?.closest('tr').style.display !== 'none' || clonedArea.querySelector('[data-item-id="liquid_analgesic_nsaid"]')?.closest('tr').style.display !== 'none') careAdviceCategories.add('MEDICATION');
    
    let careAdviceHTML = `<li>${careAdviceMap['GENERAL']}</li>`;
    ['EXTRACTION', 'RESIN', 'MEDICATION', 'RECHECK'].forEach(key => { if (careAdviceCategories.has(key)) careAdviceHTML += `<li>${careAdviceMap[key]}</li>`; });
...
function addExportListeners(pageSelector, type) {
    ...
    const exportHandler = (exportFunc) => {
        ...
        const hiddenRows = [];
        captureArea.querySelectorAll('.additional-treatments-container tr.additional-row, .main-container tr').forEach(row => {
    ...
    btnContainer.querySelector('.export-png-btn')?.addEventListener('click', () => exportHandler((canvas, fileName) => {
    ...
    btnContainer.querySelector('.export-pdf-btn')?.addEventListener('click', () => exportHandler((canvas, fileName) => {
